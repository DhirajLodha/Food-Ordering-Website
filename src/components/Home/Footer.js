import React from "react";
import "../css/Home.css";
import "../css/nicepage.css";
// import "../js/nicepage";

function Footer() {
  return (
    <footer className="u-clearfix u-footer u-grey-80" id="sec-7348">
      <div className="u-clearfix u-sheet u-sheet-1">
        <div className="u-clearfix u-expanded-width u-gutter-10 u-layout-wrap u-layout-wrap-1">
          <div className="u-gutter-0 u-layout">
            <div className="u-layout-row">
              <div className="u-align-center-sm u-align-center-xs u-align-left-md u-align-left-xl u-container-style u-layout-cell u-left-cell u-size-20 u-size-20-md u-layout-cell-1">
                <div className="u-container-layout u-valign-bottom-lg u-valign-bottom-md u-container-layout-1">
                  {/* <!--position--> */}
                  <div
                    data-position=""
                    className="u-expanded-width-xs u-position u-position-1"
                  >
                    {/* <!--block--> */}
                    <div className="u-block">
                      <div className="u-block-container u-clearfix">
                        {/* <!--block_header--> */}
                        <h2 className="u-align-center u-block-header u-text">
                          {/* <!--block_header_content--> */}
                          Contact Us
                          <span style={{ fontSize: "1.5rem" }}>
                            <span style={{ fontSize: "1.875rem" }}></span>
                          </span>
                          {/* <!--/block_header_content--> */}
                        </h2>
                        {/* <!--/block_header--> */}
                        {/* <!--block_content--> */}
                        <div className="u-align-center u-block-content u-text u-text-white u-text-2">
                          {/* <!--block_content_content--> */}
                          <br />
                          207, 1st Floor, CMR Road, 3rd&nbsp;
                          <br />
                          HRBR Layout, Kalyan Nagar, Surat - 395010
                          <br />
                          <br />
                          +91 9876543210
                          <br />
                          franchise@thepizzavilla.in
                          {/* <!--/block_content_content--> */}
                        </div>
                        {/* <!--/block_content--> */}
                      </div>
                    </div>
                    {/* <!--/block--> */}
                  </div>
                  {/* <!--/position--> */}
                </div>
              </div>
              <div className="u-container-style u-layout-cell u-size-20 u-size-20-md u-layout-cell-2">
                <div className="u-container-layout u-container-layout-2">
                  <h2 className="u-align-center u-text u-text-3">
                    Stay Connected
                  </h2>
                  <p className="u-align-center u-text u-text-4">
                    Follow us on media channels{" "}
                  </p>
                  <div className="u-social-icons u-spacing-10 u-social-icons-1">
                    <a
                      className="u-social-url"
                      title="facebook"
                      target="_blank"
                      href="/"
                    >
                      <span className="u-icon u-social-facebook u-social-icon u-text-black">
                        <svg
                          className="u-svg-link"
                          preserveAspectRatio="xMidYMin slice"
                          viewBox="0 0 112 112"
                        >
                          <use
                            xmlnsXlink="http://www.w3.org/1999/xlink"
                            xlinkHref="#svg-dc22"
                          ></use>
                        </svg>
                        <svg
                          className="u-svg-content"
                          viewBox="0 0 112 112"
                          x="0"
                          y="0"
                          id="svg-dc22"
                        >
                          <circle
                            fill="currentColor"
                            cx="56.1"
                            cy="56.1"
                            r="55"
                          ></circle>
                          <path
                            fill="#FFFFFF"
                            d="M73.5,31.6h-9.1c-1.4,0-3.6,0.8-3.6,3.9v8.5h12.6L72,58.3H60.8v40.8H43.9V58.3h-8V43.9h8v-9.2
            c0-6.7,3.1-17,17-17h12.5v13.9H73.5z"
                          ></path>
                        </svg>
                      </span>
                    </a>
                    <a
                      className="u-social-url"
                      title="twitter"
                      target="_blank"
                      href="/"
                    >
                      <span className="u-icon u-social-icon u-social-twitter u-text-black">
                        <svg
                          className="u-svg-link"
                          preserveAspectRatio="xMidYMin slice"
                          viewBox="0 0 112 112"
                        >
                          <use
                            xmlnsXlink="http://www.w3.org/1999/xlink"
                            xlinkHref="#svg-bfa2"
                          ></use>
                        </svg>
                        <svg
                          className="u-svg-content"
                          viewBox="0 0 112 112"
                          x="0"
                          y="0"
                          id="svg-bfa2"
                        >
                          <circle
                            fill="currentColor"
                            className="st0"
                            cx="56.1"
                            cy="56.1"
                            r="55"
                          ></circle>
                          <path
                            fill="#FFFFFF"
                            d="M83.8,47.3c0,0.6,0,1.2,0,1.7c0,17.7-13.5,38.2-38.2,38.2C38,87.2,31,85,25,81.2c1,0.1,2.1,0.2,3.2,0.2
            c6.3,0,12.1-2.1,16.7-5.7c-5.9-0.1-10.8-4-12.5-9.3c0.8,0.2,1.7,0.2,2.5,0.2c1.2,0,2.4-0.2,3.5-0.5c-6.1-1.2-10.8-6.7-10.8-13.1
            c0-0.1,0-0.1,0-0.2c1.8,1,3.9,1.6,6.1,1.7c-3.6-2.4-6-6.5-6-11.2c0-2.5,0.7-4.8,1.8-6.7c6.6,8.1,16.5,13.5,27.6,14
            c-0.2-1-0.3-2-0.3-3.1c0-7.4,6-13.4,13.4-13.4c3.9,0,7.3,1.6,9.8,4.2c3.1-0.6,5.9-1.7,8.5-3.3c-1,3.1-3.1,5.8-5.9,7.4
            c2.7-0.3,5.3-1,7.7-2.1C88.7,43,86.4,45.4,83.8,47.3z"
                          ></path>
                        </svg>
                      </span>
                    </a>
                    <a
                      className="u-social-url"
                      title="instagram"
                      target="_blank"
                      href="/"
                    >
                      <span className="u-icon u-social-icon u-social-instagram u-text-black">
                        <svg
                          className="u-svg-link"
                          preserveAspectRatio="xMidYMin slice"
                          viewBox="0 0 112 112"
                        >
                          <use
                            xmlnsXlink="http://www.w3.org/1999/xlink"
                            xlinkHref="#svg-a623"
                          ></use>
                        </svg>
                        <svg
                          className="u-svg-content"
                          viewBox="0 0 112 112"
                          x="0"
                          y="0"
                          id="svg-a623"
                        >
                          <circle
                            fill="currentColor"
                            cx="56.1"
                            cy="56.1"
                            r="55"
                          ></circle>
                          <path
                            fill="#FFFFFF"
                            d="M55.9,38.2c-9.9,0-17.9,8-17.9,17.9C38,66,46,74,55.9,74c9.9,0,17.9-8,17.9-17.9C73.8,46.2,65.8,38.2,55.9,38.2
            z M55.9,66.4c-5.7,0-10.3-4.6-10.3-10.3c-0.1-5.7,4.6-10.3,10.3-10.3c5.7,0,10.3,4.6,10.3,10.3C66.2,61.8,61.6,66.4,55.9,66.4z"
                          ></path>
                          <path
                            fill="#FFFFFF"
                            d="M74.3,33.5c-2.3,0-4.2,1.9-4.2,4.2s1.9,4.2,4.2,4.2s4.2-1.9,4.2-4.2S76.6,33.5,74.3,33.5z"
                          ></path>
                          <path
                            fill="#FFFFFF"
                            d="M73.1,21.3H38.6c-9.7,0-17.5,7.9-17.5,17.5v34.5c0,9.7,7.9,17.6,17.5,17.6h34.5c9.7,0,17.5-7.9,17.5-17.5V38.8
            C90.6,29.1,82.7,21.3,73.1,21.3z M83,73.3c0,5.5-4.5,9.9-9.9,9.9H38.6c-5.5,0-9.9-4.5-9.9-9.9V38.8c0-5.5,4.5-9.9,9.9-9.9h34.5
            c5.5,0,9.9,4.5,9.9,9.9V73.3z"
                          ></path>
                        </svg>
                      </span>
                    </a>
                    <a
                      className="u-social-url"
                      target="_blank"
                      data-type="YouTube"
                      title="YouTube"
                      href="/"
                    >
                      <span className="u-icon u-social-icon u-social-youtube u-text-black u-icon-4">
                        <svg
                          className="u-svg-link"
                          preserveAspectRatio="xMidYMin slice"
                          viewBox="0 0 112 112"
                        >
                          <use
                            xmlnsXlink="http://www.w3.org/1999/xlink"
                            xlinkHref="#svg-0871"
                          ></use>
                        </svg>
                        <svg
                          className="u-svg-content"
                          viewBox="0 0 112 112"
                          x="0"
                          y="0"
                          id="svg-0871"
                        >
                          <circle
                            fill="currentColor"
                            cx="56.1"
                            cy="56.1"
                            r="55"
                          ></circle>
                          <path
                            fill="#FFFFFF"
                            d="M74.9,33.3H37.3c-7.4,0-13.4,6-13.4,13.4v18.8c0,7.4,6,13.4,13.4,13.4h37.6c7.4,0,13.4-6,13.4-13.4V46.7 C88.3,39.3,82.3,33.3,74.9,33.3L74.9,33.3z M65.9,57l-17.6,8.4c-0.5,0.2-1-0.1-1-0.6V47.5c0-0.5,0.6-0.9,1-0.6l17.6,8.9 C66.4,56,66.4,56.8,65.9,57L65.9,57z"
                          ></path>
                        </svg>
                      </span>
                    </a>
                  </div>
                </div>
              </div>
              <div className="u-align-center-sm u-align-center-xs u-align-left-md u-align-right-lg u-align-right-xl u-container-style u-layout-cell u-right-cell u-size-20 u-size-20-md u-layout-cell-3">
                <div className="u-container-layout u-container-layout-3">
                  <div className="fr-view u-clearfix u-rich-text u-text u-text-5">
                    <h2 style={{ textAlign: "center" }}>Timings</h2>
                    <p style={{ textAlign: "center" }}>
                      <span style={{ fontSize: "1.25rem" }}>
                        Monday - Sunday
                      </span>
                    </p>
                    <p style={{ textAlign: "center", lineHeight: "1" }}>
                      <span style={{ fontSize: "1.25rem" }}>
                        9:00 AM - 11:00 PM
                      </span>
                    </p>
                    <p style={{ textAlign: "center" }}>
                      <br />
                    </p>
                    <p style={{ textAlign: "center" }}>
                      <br />
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <p className="u-align-center u-text u-text-6">
          Copyright © 2022 All Rights Reserved By The Pizza Villa
        </p>
      </div>
    </footer>
  );
}

export default Footer;
